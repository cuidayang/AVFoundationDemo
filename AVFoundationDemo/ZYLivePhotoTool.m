...
- (void)generatorLivePhotoWithAsset:(AVAsset *)asset
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [self dealVideoWithWriteFilePath:videoPath AssetIdentifier:assetID asset:asset progress:progress];
...
- (void)dealVideoWithWriteFilePath:(NSString *)finalMovPath
    ...
    dispatch_async(createMovQueue, ^{
        int i = 0;
        int total = 30 * CMTimeGetSeconds(asset.duration);
        while (reader.status == AVAssetReaderStatusReading) {
            ...
            if (videoBuffer) {
                while (!videoInput.isReadyForMoreMediaData || (audioInput && !audioInput.isReadyForMoreMediaData)) {
                    usleep(1);
                }
